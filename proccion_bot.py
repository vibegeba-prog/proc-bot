...
def generate_evening_post(sunset_time: str | None) -> str:
    ...
    if sunset_time and sunset_already_passed(sunset_time):
        templates = [
            ...
            f"🌆 Вечер в {AREA_PREP}. Окна горят тёплым светом — приятно смотреть с улицы. Отдохните хорошо. Ваш УК Процион.",
    ...
    if args.mode == "morning":
        if OPENWEATHER_KEY:
            ...
            if weather:
                ...
                text = "☀️ Всем, кто проснулся на " + STREET_PREP + ", — чудесного дня и отличного настроения. Ваш УК Процион.\n\n#УКПроцион #ЖКМурино #Бугры #Мурино"
        else:
