...
Автоматические посты в канал:
  ...
  • Вечер (20:30) — каждый будний день: закат + пожелание
    (если солнце уже село — пишем без упоминания времени)
...
#УКПроцион #ЖКМурино #Бугры #Мурино #вечер"
    ...
    # Если закат ещё впереди или не удалось получить — пишем с временем
    if sunset_time:
        templates = [
    ...
    if args.mode == "morning":
        if OPENWEATHER_KEY:
            ...
            if weather:
                ...
                text = f"☀️ Всем, кто проснулся на {STREET}, — чудесного дня и отличного настроения. Ваш УК Процион.
    ...
    if args.mode == "evening":
        ...
        if sunset:
            ...
            if sunset_already_passed(sunset):
                print("[INFO] Солнце уже село — используем пост без времени заката")
            else:
