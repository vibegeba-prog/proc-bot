...
# Правильные падежи для склонений
AREA_PREP    = "Буграх"              # в Буграх, о Буграх
AREA_GEN     = "Бугров"              # из Бугров
STREET_PREP  = "Воронцовском бульваре"  # на Воронцовском бульваре
STREET_DAT   = "Воронцовскому бульвару"  # по Воронцовскому бульвару
STREET_GEN   = "Воронцовского бульвара"  # с Воронцовского бульвара
...
def get_sunset():
    ...
    try:
        ...
        # Явно конвертируем в московское время (UTC+3)
        sunset_msk = sunset_utc.astimezone(tz=timezone(timedelta(hours=3)))
...
def generate_morning_post(weather: dict) -> str:
    ...
    if "дождь" in desc or "ливень" in desc or "гроза" in desc:
        ...
        templates = [
            f"{emoji} Снегопад в {AREA_PREP}, +{temp}°. На {STREET_PREP} уже бело — будьте осторожны на дорогах и не спешите. Тёплого вам дня!",
            f"{emoji} Белое утро в {COMPLEX_NAME}, +{temp}°. Снег красивый, но скользкий. Выходите из дома в удобной обуви. Всего хорошего!",
        ]
    elif temp >= 25:
        templates = [
            f"{emoji} Жара на {STREET_PREP}: +{temp}°! Отличный повод проветрить квартиру с утра, пока не началась дневная жара. Пейте больше воды. Продуктивного дня!",
        ...
        templates = [
            f"{emoji} Холодное утро на {STREET_PREP}: +{temp}°. Не забудьте шапку — ветер {wind} м/с. Подъезды тёплые, лифты работают. Удачного дня!",
...
def generate_evening_post(sunset_time: str | None) -> str:
    ...
    if sunset_time and sunset_already_passed(sunset_time):
        templates = [
            f"🌙 Вечер на {STREET_PREP}. Надеемся, ваш день прошёл хорошо. Завтра встретимся снова — в 8:45. Тихой ночи!",
            f"🌃 {COMPLEX_NAME} засыпает. Если вы ещё на работе — скорее домой, тут тепло и уютно. До завтра!",
            f"🌆 Вечер в {AREA_PREP}. Окна горят тёплым светом — приятно смотреть с улицы. Отдохните хорошо. Ваша УК Процион.",
            f"⭐ Небо над {AREA} уже тёмное. Время чая, книги или любимого сериала. Завтра новый день. Спокойной ночи!",
            f"🌙 {STREET_PREP} тихий и спокойный. Мы желаем вам тёплого пледа и сладких снов. До встречи утром!",
        ]
        post = random.choice(templates)
    ...
    if args.mode == "morning":
        if OPENWEATHER_KEY:
            ...
            if weather:
                ...
                text = "☀️ Всем, кто проснулся на " + STREET_PREP + ", — чудесного дня и отличного настроения. Ваша УК Процион.\n\n#УКПроцион #ЖКМурино #Бугры #Мурино"
        else:
