...
Автоматические посты в канал:
  • Утро (8:45) — пн–пт: погода + тёплое слово
  • Вечер (20:30) — пн–пт: закат + пожелание
...
# Правильные падежи
AREA_PREP    = "Буграх"
STREET_PREP  = "Воронцовском бульваре"
STREET_DAT   = "Воронцовскому бульвару"
STREET_GEN   = "Воронцовского бульвара"
...
# ============================================================
# 3. ЗАКАТ (с московским временем)
# ============================================================

def get_sunset():
    """Получает время заката в районе ЖК Мурино (по московскому времени)."""
    url = (
...
# ============================================================
# 4. УТРЕННИЕ ПОСТЫ — без повторов, живой язык
# ============================================================
...
def generate_morning_post(weather: dict) -> str:
    ...
    if "дождь" in desc or "ливень" in desc or "гроза" in desc:
        templates = [
            ...
            f"{emoji} Дождливое утро в {AREA_PREP}, +{temp}°. Захватите сменную обувь, а мы пока будем надеяться, что к обеду разойдётся. Удачного дня!",
        ]
    elif "снег" in desc or "снегопад" in desc:
        templates = [
            f"{emoji} Снегопад в {AREA_PREP}, +{temp}°. На {STREET_PREP} уже бело — будьте осторожны на дорогах и не спешите. Тёплого дня!",
            f"{emoji} Белое утро в {COMPLEX_NAME}, +{temp}°. Снег красивый, но скользкий. Выходите из дома в удобной обуви. Доброго дня!",
        ]
    elif temp >= 25:
        templates = [
            f"{emoji} Жара на {STREET_PREP}: +{temp}°! Отличный повод проветрить квартиру с утра, пока не начался зной. Пейте больше воды. Лёгкого дня!",
            f"{emoji} +{temp}° — лето в разгаре. Идеальный день для вечерней прогулки по окрестностям, а пока — тень и прохлада. Бодрого дня!",
        ]
    elif temp >= 18:
        templates = [
            f"{emoji} +{temp}° — погода, ради которой стоит проснуться пораньше. Ветер {wind} м/с, на {STREET_PREP} сейчас особенно свежо. Прекрасного дня!",
            f"{emoji} Прекрасное утро в {COMPLEX_NAME}: +{temp}°. Окна на запад открывайте сейчас — до обеда будет прохладно и тихо. Удачного дня!",
        ]
    elif temp >= 10:
        templates = [
            f"{emoji} Прохладно, +{temp}°, {desc}. Ветер {wind} м/с — наденьте кофту. Ничего, скоро лето. Приятного дня!",
            f"{emoji} +{temp}° в {AREA_PREP}. Свежо, как после дождя. Чашка кофе — и вперёд. Бодрого дня!",
        ]
    elif temp >= 0:
        templates = [
            f"{emoji} Холодное утро на {STREET_PREP}: +{temp}°. Не забудьте шапку — ветер {wind} м/с. Подъезды тёплые, лифты работают. Счастливого дня!",
...
def generate_evening_post(sunset_time: str | None) -> str:
    ...
    if sunset_time and sunset_already_passed(sunset_time):
        templates = [
            f"🌙 Вечер на {STREET_PREP}. День прошёл — отлично. Завтра встретимся в 8:45. Сладких снов!",
            f"🌃 {COMPLEX_NAME} засыпает. Если вы ещё на работе — скорее домой, тут тепло и уютно. До завтра!",
            f"🌆 Вечер в {AREA_PREP}. Окна горят тёплым светом — приятно смотреть с улицы. Отдохните хорошо.",
            f"⭐ Небо над {AREA} уже тёмное. Время чая, книги или любимого сериала. Завтра новый день. Спокойной ночи!",
            f"🌙 {STREET_PREP} тихий и спокойный. Желаем вам тёплого пледа и сладких снов. До встречи утром!",
        ]
        post = random.choice(templates)
    ...
    if sunset_time:
        templates = [
            f"🌅 Сегодня солнце сядет в {sunset_time}. Если у вас окна на запад — остановитесь на минутку: вид с {STREET_GEN} в этот час стоит того. Доброй ночи!",
            f"🌇 Закат сегодня в {sunset_time}. Рабочий день позади — время для себя и близких. Отдохните хорошо, завтра встретимся снова.",
            f"🌙 Сегодняшний закат в {sunset_time}. Вечера в {AREA_PREP} становятся уютнее с каждым днём. Желаем вам тёплого пледа, чая и сладких снов.",
            f"🌆 Закат в {sunset_time}. Прогуляйтесь вечером по {STREET_DAT} — свежий воздух после жаркого дня и красивый небосвод. Приятного вечера!",
            f"🌠 Сегодня солнце сядет в {sunset_time}. Если вы ещё в дороге — не спешите: небо над {AREA} сейчас особенное. Сладких снов, {COMPLEX_NAME}!",
        ]
    else:
        templates = [
            f"🌙 Вечер на {STREET_PREP}. День прошёл — отлично. Завтра встретимся в 8:45. Сладких снов!",
    ...
    if args.mode == "morning":
        if OPENWEATHER_KEY:
            ...
            if weather:
                ...
                text = "☀️ Всем, кто проснулся на " + STREET_PREP + ", — чудесного дня и отличного настроения.\n\n#УКПроцион #ЖКМурино #Бугры #Мурино"
        else:
